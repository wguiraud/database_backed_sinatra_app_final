...
configure :development do
  ...
  require_relative 'config/environments/development' if development?
  enable :sessions
  set session_secret: ENV['SESSION_SECRET']
  set :erb, escape_html: true
end
...
#if the connection with the db is not proper display send back to the client
# an standard sinatra error message
error do
...
get '/db-test' do
  result = @storage.query('SELECT 1 as test')
  "Database connected: #{result.first['test']}"
